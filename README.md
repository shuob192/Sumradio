...
生音声は入力のネイティブレート・最大2チャンネル・float32で保存。ASR用音声は16 kHz・モノラル・16-bit PCMで保存します。ライブ入力は最大30秒で分割。WAV取込は8〜192 kHz、1〜2ch、2分・16 MiB以下です。録音済みWAVは確定認識のみで、暫定字幕はライブ入力と台本再生で表示します。
